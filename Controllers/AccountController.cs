    ...
    : ControllerBase
{
    ...
    public async Task<IActionResult> Login(LoginDto loginDto)
    {
        ...
        return Ok(new NewUserDto
            { UserName = user.UserName!, Email = user.Email!, Role = role!, Token = tokenService.GenerateToken(user) });
    }
        ...
        try
        {
            ...
            if (createUser.Succeeded)
            {
                ...
                if (roleResult.Succeeded)
                    return Ok(new NewUserDto
                    {
                        Email = registerDto.Email, UserName = registerDto.UserName, Role = role,
